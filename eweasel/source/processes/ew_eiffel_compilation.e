...
feature {NONE} -- Creation
	...
	make (cmd: READABLE_STRING_32; args: LIST [READABLE_STRING_32]; env_vars: like {EW_TEST_ENVIRONMENT}.environment_variables; save: READABLE_STRING_32)
	...
	abort
		...
		do
			...
			if suspended then
				...
				next_compile_result.do_nothing
			end
				...
				loop
					...
					if not end_of_file then
						...
						if last_char /= '%R' then
							...
							count := count + 1
							is_resume_prompt := count = Resume_prompt.count and then line.same_string (Resume_prompt)
							is_missing_precomp := count = Missing_precompile_prompt.count and then line.same_string (Missing_precompile_prompt)
